...
export default function RecommendationTable() {
  ...
  // Fetch CMP for stocks only, every 2 seconds
  useEffect(() => {
    async function fetchStockPrice(symbol) {
      try {
        await fetch(
        ...
        recommendations.map(async (rec) => {
          try {
            if (rec.assetType === 'Stock') {
              // Get instrument_token from backend
              const res = await axiosInstance.get(
                `/v1/stocks/get/${rec.mutualFund}`
              )
              const instrument_token =
                res.data.data.instrument_token
              // Fetch live price
              ...
              // Get NAV from backend
              const res = await axiosInstance.get(
                `/v1/mutual-funds/get/${rec.mutualFund}`
              )
              prices[rec.mutualFund] = res.data.data.nav
            }
            console.log(prices)
          } catch (error) {
            console.error(
    ...
    fetchAllCMPs()
    intervalRef.current = setInterval(fetchAllCMPs, 2000)
  ...
  const AssetNameCellRenderer = (params) => {
    return (
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="font-semibold text-gray-900 group-hover:text-[#00d09c] transition-colors duration-200">
            {params.value}
          </p>
  ...
  const CMPRenderer = (params) => {
    const rec = params.data
    const cmp = cmpMap[rec.mutualFund]
    return (
      <span className="text-sm text-gray-800 font-medium">
        {cmp !== null && cmp !== undefined ? `₹${cmp}` : '—'}
      </span>
    )
  }
      ...
      <div className="max-w-9xl mx-auto px-2 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-3xl font-bold text-[var(--dashboard-text-dark)]">
                Investment Recommendations
              </h1>
              <p className="text-gray-600 mt-1">
                Manage and track your investment recommendations
              </p>
            </div>
          </div>
