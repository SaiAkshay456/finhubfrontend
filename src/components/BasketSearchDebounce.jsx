...
//         router.push(`?${params.toString()}`);
//         setTimeout(() => {
//             setLoading(false); // simulate end of loading (or set this from response elsewhere)
//         }, 400); // adjust this if your data load is async
//     }, 100);
...
export default function BasketSearchDebounce({
    ...
    const debouncedSearchUpdate = useCallback((value) => {
        setLoading(true);
        // ALWAYS use the 'searchParams' from the hook, not window.location
        ...
        // This timeout is just for visual feedback, can be removed
        setTimeout(() => {
            setLoading(false);
        }, 400);
    }, [searchParams, router]);
    ...
    useEffect(() => {
        const handler = setTimeout(() => {
            // Only trigger the update if the search term in state is different from the one in the URL
    ...
    }, [search, searchParams, debouncedSearchUpdate]);

    // Sync state with URL if the user navigates (e.g., using back/forward buttons)
    useEffect(() => {
        ...
        <form onSubmit={(e) => e.preventDefault()} className="flex flex-col sm:flex-row gap-4">
            ...
            {search && (
                <button
                    ...
                    onClick={() => {
                        setSearch('');
                        // Use the reactive searchParams from the hook here as well
                        const params = new URLSearchParams(searchParams.toString());
                        params.delete('search');
                        params.set('page', '1');
                        router.push(`?${params.toString()}`);
