...
export default function BasketSearchDebounce({ placeholder }) {
    const searchParams = useSearchParams();
    const router = useRouter();
    const handleSearch = useDebouncedCallback((term) => {
        ...
        // When a user starts a new search, it's best practice to reset them to page 1
        params.set('page', '1');

        // Push the updated URL. The router will handle the navigation.
        // The '?' is automatically added by the router.
        router.push(`?${params.toString()}`);
    ...
    return (
        <input
            type="text"
            // Use defaultValue to set the initial value from the URL without making it a controlled component
            defaultValue={searchParams.get('search') || ''}
            onChange={(e) => {
                handleSearch(e.target.value);
            }}
            placeholder={placeholder}
            className="w-full px-4 py-2 text-sm text-gray-800 bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-shadow duration-200"
        />
    );
}
